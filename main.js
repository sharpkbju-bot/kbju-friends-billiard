// main.js - V9.80 Live Pulse Edition
...
function pickRandomOrder() {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        // [V9.77 신규 추가 로직] 직전 게임 초구자가 또 1번(초구)로 배치되면 극단적 페널티 부여하여 강제 배제
...
// [V9.80] renderLastPlaceRecord 함수를 최하단에서 호출하도록 로직 영향도 완벽 유지
function renderAll() { 
...
// [V9.75 신규] 월간 종합 시상식 (Monthly Awards) 로직 - MVP 타이브레이커 추가
function showMonthlyAwards() {
    ...
    if (!monthVal) {
        showToastMsg('"월 선택" 버튼을 클릭하여 월을 선택하세요.');
        return;
    }
...
// [V9.80] 신규 로직 추가: 선수별 마지막 최하위 기록(꼴찌) 탐색 및 산출 독립 함수 (어떠한 최적화/변수 공유도 없음)
function renderLastPlaceRecord() {
    ...
    sortedGames.forEach(g => {
        ...
        // 혼자 게임한 경우 등은 꼴찌로 카운트하지 않음 방어 로직
    ...
    // 정렬 로직: 무패자 최상단 -> 무사고 생존 게임수가 많은 순서대로
    ...
    // 날짜 차이 계산을 위해 타임존 오류를 없애고 자정으로 강제 통일
