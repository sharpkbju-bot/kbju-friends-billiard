...
let defenseModalTimeout = null; // 디펜스 모달용 타이머 추가
...
// v5.55 디펜스 상세 기록 팝업 렌더링 함수
function showDefenseDetail(playerName) {
    let victimStats = {}; // 특정 선수 뒤에서 친 '희생양'들의 통계
    ...
    gameLogs.forEach(g => {
        if (g.startOrder && g.startOrder.includes(playerName)) {
            ...
            const victimName = order[(pIdx + 1) % order.length]; // 내 바로 다음 순서 선수
    ...
    let html = `<div id="defense-modal-capture-area" style="padding: 10px; border-radius: 15px; background: transparent;">
                ...
                <h2 style="font-size:19px; font-weight:900; color:var(--text-color); margin:0 0 5px 0;">${playerName}의 방어 리포트</h2>
                <div style="font-size:13px; font-weight:800; color:var(--sub-text); margin-bottom: 20px;">(내 바로 뒷주자 선수들의 성적 분석)</div>
    ...
    defenseModalTimeout = setTimeout(() => { closeDefenseDetail(); }, 15000); // 15초 후 자동 닫힘
}
