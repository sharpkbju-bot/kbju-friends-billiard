...
// 💡 겐세이 MVP 로직 추가 (V5.28 베이스에서 오직 이 함수만 수정)
function renderTodayMVP() {
    ...
    gamesToday.forEach(g => { 
        ...
        // 겐세이 분석: 시작 순서(startOrder)가 저장된 경우에만 실행
        if (g.startOrder && g.startOrder.length > 1) {
            const order = g.startOrder;
            for (let i = 0; i < order.length; i++) {
                const p1 = order[i]; // 앞사람 (겐세이 주체)
                const p2 = order[(i + 1) % order.length]; // 뒷사람 (피해자)
                const p2RankIdx = actual.indexOf(p2);

                if (p2RankIdx !== -1) {
                    if (!genseiStats[p1]) genseiStats[p1] = { causedLasts: 0, sumRank: 0, count: 0 };
                    genseiStats[p1].count++;
                    genseiStats[p1].sumRank += (p2RankIdx + 1); // 평균 등수 계산용 합산

                    // 뒷사람이 꼴찌를 했다면
                    if (p2RankIdx === actual.length - 1 && actual.length > 1) {
                        genseiStats[p1].causedLasts++;
                    }
                }
            }
        }
    ...
    // 겐세이 MVP 선정 로직
    let genseiKing = null;
    const genseiCandidates = Object.keys(genseiStats).filter(p => genseiStats[p].count > 0);

    if (genseiCandidates.length > 0) {
        genseiKing = genseiCandidates.reduce((a, b) => {
            const sA = genseiStats[a];
            const sB = genseiStats[b];

            // 1순위: 뒷사람을 꼴찌로 만든 횟수가 더 많은 사람
            if (sA.causedLasts !== sB.causedLasts) {
                return sA.causedLasts > sB.causedLasts ? a : b;
            }
            // 2순위: 뒷사람의 평균 등수가 더 낮은(숫자가 더 큰) 사람
            const avgA = sA.sumRank / sA.count;
            const avgB = sB.sumRank / sB.count;
    ...
    // 😈 겐세이 뱃지 추가 렌더링
    if (genseiKing && genseiStats[genseiKing]) {
        html += `<div class="mvp-badge">
                      <span class="mvp-title">😈 겐세이</span>
                      <span class="mvp-player" style="color: #9c27b0;">${genseiKing}</span>
                      <span class="mvp-value">뒷사람 꼴찌 ${genseiStats[genseiKing].causedLasts}회</span>
                  </div>`;
    }
...
function importData(event) {
    ...
    reader.onload = async function(e) {
        try {
            ...
            if (!confirm(`백업 파일에서 ${importedData.length}개의 데이터를 발견했습니다.\n전체 복구를 진행하시겠습니까?\n\n(기존 데이터를 초기화하고 개별 저장되므로 시간이 다소 소요될 수 있습니다. 복구 중 창을 닫지 마세요.)`)) {
                event.target.value = '';
                return;
            }
            ...
            for (let i = 0; i < importedData.length; i++) {
                ...
                const payload = { 
                    ...
                    date: game.dateStr, 
                    ranks: [ranks[0] || "", ranks[1] || "", ranks[2] || "", ranks[3] || "", ranks[4] || ""], 
                    startOrder: game.startOrder || null 
                };
