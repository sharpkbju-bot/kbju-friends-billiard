...
let genseiCountdownInterval = null; // v6.10: 10초 라이브 카운트다운을 위해 Interval로 변경
let defenseModalTimeout = null; 
let infoModalCountdownInterval = null; // v6.10: 10초 라이브 카운트다운을 위해 Interval로 변경
...
async function captureAndShare(targetId, btnId, fileName, shareTitle, shareText) {
    ...
    // [수정 포인트] 확대 모드에서 팝업 모달 캡처 시 글자 겹침 현상을 방지하기 위해
    // 캡처 직전 실제 DOM의 확대를 풀고, 캡처 직후 다시 복구합니다.
    const originalZoom = document.body.style.zoom;
    const isZoomActive = document.body.classList.contains('zoom-active');
    if (isZoomActive) {
    ...
    try {
        ...
        // [복구] 캡처 완료 후 확대 상태 원상 복구
        if (isZoomActive) {
            document.body.style.zoom = originalZoom || '1.2';
...
function showRingCriteria(type) {
    ...
    if (type === 'win') {
        ...
        title = "평균득점(🔴) 산출 기준";
        desc = "<b>(해당 월 평균 승점 / 3.0) × 100</b><br><br>3인 게임 1위 만점(3.0)을 기준으로 본인의 평균 승점 수준을 나타냅니다. 꾸준한 기량을 보여주는 지표입니다.";
    } else if (type === 'safety') {
        title = "생존율(🟢) 산출 기준";
    ...
    document.getElementById('info-modal-icon').innerHTML = "🍎";
...
function showInfoModal(type) {
    ...
    const timerEl = document.getElementById('info-modal-timer');
    if(timerEl) timerEl.style.display = 'none'; // 일반 정보 모달에서는 10초 타이머 숨김
...
function showDefenseDetail(playerName) {
    ...
    let html = `<div id="defense-modal-capture-area" style="padding: 10px; border-radius: 15px; background: transparent;">
                ...
                <div style="font-size:13px; font-weight:800; color:var(--sub-text); margin-bottom: 20px; line-height:1.4; white-space:nowrap;">(내&nbsp;바로&nbsp;뒷주자&nbsp;선수들의&nbsp;성적&nbsp;분석)</div>
                <div style="display:flex; flex-direction:column; gap:10px;">`;

    if (victims.length === 0) {
        html += `<div style="padding:30px; color:var(--sub-text); font-weight:800;">분석 가능한 데이터가 없습니다.</div>`;
    } else {
        victims.forEach(v => {
            ...
            html += `<div style="background:rgba(255,255,255,0.4); padding:12px 15px; border-radius:15px; border:1px solid rgba(0,0,0,0.05); text-align:left;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                            <span style="color:${getPlayerColor(v)}; font-size:17px; font-weight:900;">${playerThemes[v].emoji} ${v}</span>
                            <span style="font-size:13px; font-weight:800; color:var(--sub-text);">${s.games}전 / 평균 ${avg}위</span>
                        </div>
                        <div style="display:flex; gap:10px; font-size:12px; font-weight:900;">
                            <div style="flex:1; background:var(--bg); padding:6px; border-radius:8px; text-align:center;">
                                <span style="color:var(--sub-text);">1위 확률</span><br>
                                <span style="color:var(--rank1); font-size:14px;">${winP}%</span>
                            </div>
                            <div style="flex:1; background:var(--bg); padding:6px; border-radius:8px; text-align:center;">
                                <span style="color:var(--sub-text);">꼴찌 확률</span><br>
                                <span style="color:var(--rankL); font-size:14px;">${lastP}%</span>
                            </div>
                        </div>
                     </div>`;
        });
...
// 겐세이 모달 오픈 (v6.10 10초 자동 닫기 타이머 추가)
function showGenseiModal(playerName) {
    ...
    html += `</div>
             ...
             <div id="gensei-countdown-text" style="margin-top:15px; font-size:12px; color:#999; font-weight:800; text-align:center;">10초 후 자동으로 닫힙니다.</div>`;
...
// v6.10 활동 링 생성 함수 
function createActivityRing(percent, color, size, label, onClickType) {
    const radius = 35; const circumference = 2 * Math.PI * radius;
    const offset = circumference - (percent / 100) * circumference;
    return `<div class="ring-container" style="width:${size}px; height:${size+25}px;" onclick="showRingCriteria('${onClickType}')">
                <svg width="${size}" height="${size}" viewBox="0 0 100 100">
                    ...
                    <text x="50" y="55" text-anchor="middle" class="ring-text" style="fill:${color}; font-weight:900;">${percent}%</text>
                </svg>
...
function searchRecords() {
    ...
    let avgScoreRate = Math.min(100, Math.round((parseFloat(monthlyAvgScore) / 5.0) * 100)); 
    ...
    // 활동 링 컬러 변경 적용: 승률 #9b59b6 (퍼플), 평균득점 #ff69b4 (핑크), 생존율 #3498db (블루)
    sArea.innerHTML = `<div class="summary-box" style="margin: 0 -5px; box-sizing: border-box; background:var(--record-bg); border:2px solid var(--record-border); border-radius:15px; padding:15px 10px;">
                           ...
                           <div class="activity-rings-row">
                               ${createActivityRing(winRate, '#9b59b6', 80, '승률', 'win')}
                               ${createActivityRing(avgScoreRate, '#ff69b4', 80, '평균득점', 'score')}
                               ${createActivityRing(safetyRate, '#3498db', 80, '생존율', 'safety')}
                           </div>
