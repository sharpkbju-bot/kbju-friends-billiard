...
function renderStats() {
    // 💡 결함 수정(2): 누적 전적 타이틀을 모드에 따라 횟수/확률로 다이내믹하게 변경
    const subtitleEl = document.querySelector('.stats-card h2 span');
    if (subtitleEl) {
    ...
    document.getElementById('statsBody').innerHTML = sortedByWin.map((p, index) => {
        ...
        // 💡 결함 수정(2): 캡처 시 글자 겹침을 방지하기 위해 셀 내부의 '%' 기호를 제거
...
function searchRecords() {
    ...
    // 💡 결함 수정(1): 날짜별 기록 검색 시 내림차순(최신순)으로 정렬되도록 로직 변경
