...
function showDashInfo(type) {
    ...
    if (type === 'totalGames') {
        ...
        if (type === 'firstBreak') {
            ...
            desc = wrapStart + "순서 추 추첨에서 <b>1번(초구)</b>으로 가장 많이 뽑힌 선수.<br>시작부터 게임을 리드하는 행운의 사나이!" + wrapEnd;
...
// [V9.82 신규] 우측 고정 사이드바(Quick Navigation Bar) 제어 로직
...
function scrollToTarget(targetId) {
    ...
    if (targetElement) {
        // 대시보드 등 display: none 상태로 숨겨져 있을 경우 에러 방어 처리
    ...
    // 이동 후 모바일 화면 시야 방해를 막기 위해 사이드바 즉시 닫기
    const sidebarWrap = document.getElementById('quick-sidebar-wrap');
    if (sidebarWrap) {
