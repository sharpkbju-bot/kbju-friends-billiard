...
async function captureAndShare(targetId, btnId, fileName, shareTitle, shareText) {
    ...
    // [V6.10 픽스] 라이브 DOM을 건드리지 않고 오직 onclone 내부에서만 조작합니다!
    ...
    try {
        ...
        const canvas = await html2canvas(target, { 
            ...
            useCORS: true,
            onclone: (clonedDoc) => {
                // 확대 모드로 인한 레이아웃 깨짐을 방지하기 위해 복제된 문서의 zoom을 초기화
                if (clonedDoc.body) {
                ...
                if (clonedTarget) {
                    // 가장 중요한 픽스: flex가 축소되어 글자가 겹치는 것을 막기 위해 명시적 너비를 강제 부여!
                    clonedTarget.style.width = '340px'; 
                    clonedTarget.style.minWidth = '340px';
...
// [수정 포인트] html2canvas가 텍스트를 겹치지 않게 하도록 span + br 구조를 확실한 div 블록 구조로 변경했습니다!
function showDefenseDetail(playerName) {
    ...
    let html = `<div id="defense-modal-capture-area" style="padding: 10px; border-radius: 15px; background: transparent;">
                ...
                <div style="font-size:19px; font-weight:900; color:var(--text-color); margin-bottom:5px; line-height:1.4;">${playerName}의 방어 리포트</div>
                <div style="font-size:13px; font-weight:800; color:var(--sub-text); margin-bottom: 20px; line-height:1.4;">(내 바로 뒷주자 선수들의 성적 분석)</div>
...
// [수정 포인트] html2canvas 겹침 방지를 위해 div 블록 구조 적용
...
function showGenseiModal(playerName) {
    ...
    let html = `<div style="font-size:40px; margin-bottom:10px;">😈</div>
                <div style="font-size:18px; font-weight:900; color:var(--text-color); margin-bottom:5px;">${playerName}의 겐세이 희생양들</div>
...
function searchRecords() {
    ...
    sArea.innerHTML = `<div class="summary-box" style="margin: 0 -5px; box-sizing: border-box; background:var(--record-bg); border:2px solid var(--record-border); border-radius:15px; padding:15px 10px;">
                           ...
                           <div class="summary-stats-grid">
                               ...
                               <div class="stat-item"><div style="margin-bottom:4px;">총/평균 승점</div><div style="font-size:15px; font-weight:900; color:var(--text-color);">${monthlyTotalScore}점 <span style="font-size:11px; display:inline;">(${monthlyAvgScore})</span></div></div>
