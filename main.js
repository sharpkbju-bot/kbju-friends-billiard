...
function showPlayersGraph(players) {
    ...
    if (window.graphTimeout) clearTimeout(window.graphTimeout); 
    window.graphTimeout = setTimeout(() => { closeGraphModal(); }, 10000);
}

function closeGraphModal() { 
    document.getElementById('graph-modal').style.display = 'none'; 
    if (window.graphTimeout) { clearTimeout(window.graphTimeout); window.graphTimeout = null; } 
