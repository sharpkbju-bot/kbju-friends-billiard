...
async function captureAndShare(targetId, btnId, fileName, shareTitle, shareText) {
    ...
    // 확대 줌 해제 후 약간의 대기시간을 주어 브라우저가 레이아웃을 다시 계산하도록 강제
    ...
    try {
        await new Promise(r => setTimeout(r, 150)); // 브라우저 렌더링 리플로우 대기
...
// html2canvas 버그 해결: flex-direction: column 대신 block 구조로 완전 대체
...
// [V6.10 완벽 픽스] html2canvas 글자 겹침 방지를 위해 flex gap을 제거하고 display: block 구조로 재편!
