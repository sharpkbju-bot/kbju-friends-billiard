...
/* 수정: 결함 해결을 위해 서버 응답을 대기하고 강제 새로고침을 포함한 복구 로직 강화 */
function importData(event) {
    ...
    reader.onload = async function(e) {
        try {
            ...
            if (!confirm(`백업 파일에서 ${importedData.length}개의 데이터를 발견했습니다.\n복구를 진행하시겠습니까?\n(기존 데이터는 백업 파일의 내용으로 모두 교체됩니다.)`)) {
                event.target.value = '';
                return;
            }

            showLoading(true, "서버 데이터 복구 중...");
            ...
            if (response.ok) {
                alert("서버 복구가 완료되었습니다. 데이터를 다시 불러옵니다.");
                event.target.value = '';
                // 서버 업데이트 반영 시간을 위해 아주 잠깐 대기 후 fetch
                setTimeout(async () => {
                    await fetchData();
                }, 1000);
            } else {
            ...
            alert("복구 중 오류가 발생했습니다. 파일 형식을 확인해주세요.");
            showLoading(false);
            event.target.value = '';
        }
    };
