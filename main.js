...
async function captureAndShare(targetId, btnId, fileName, shareTitle, shareText) {
    ...
    // 확대 모드일 경우 강제로 기본 모드(1.0)로 원복
    const wasZoomActive = document.body.classList.contains('zoom-active');
    if (wasZoomActive) {
        changeZoom('1');
        // 브라우저가 레이아웃을 즉시 다시 계산하도록 강제
        void document.body.offsetHeight; 
    }

    // 완전히 1.0 비율로 렌더링될 때까지 충분한 시간 대기 (두 번째 캡처 에러 방지)
    ...
    try {
        ...
        // Promise를 활용해 Blob 생성이 완료될 때까지 대기
        ...
        // 캡처 처리가 전부 끝난 뒤 확대 모드 원복 (이로써 다음 캡처 시 상태 꼬임 방지)
        if (wasZoomActive) {
            changeZoom('1.2');
        }
    }
