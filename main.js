main_js_content = r"""let scoreModalTimeout = null;
...
// [V6.10 캡처 버그 최종 완벽 픽스] Ghost Wrapper 기법 도입
// 캡처 대상 요소를 화면 밖 절대 좌표에 '강제 고정 크기'로 복제하여
// 모달창, 줌 모드, Flexbox 구조에서 발생하는 html2canvas의 모든 레이아웃 붕괴와 글자 겹침을 원천 차단합니다.
async function captureAndShare(targetId, btnId, fileName, shareTitle, shareText) {
    ...
    // html2canvas가 오차 없이 렌더링할 수 있는 가장 완벽하고 깨끗한 환경을 강제합니다.
    const ghostWrapper = document.createElement('div');
    ghostWrapper.style.position = 'absolute';
    ghostWrapper.style.top = '-9999px';
    ghostWrapper.style.left = '0';
    ghostWrapper.style.width = '360px'; // 모바일 카드 최적 사이즈 강제 고정
    ...
    // 4. select, input 폼 요소를 일반 div 텍스트로 변환 (html2canvas 폼 렌더링 오류 방지)
    ...
    try {
        // 브라우저가 고스트 요소를 화면에 완전히 렌더링하고 계산할 시간(300ms) 부여
        // 이 대기 시간이 없으면 두 번째 캡처부터 레이아웃이 꼬일 수 있습니다.
...
function showRingCriteria(type) {
    ...
    if (type === 'win') {
        title = "승률(🔵) 산출 기준";
        desc = "<b>(1위 횟수 / 참여 경기수) × 100</b><br><br>해당 월에 참여한 전체 경기 중 1위를 차지한 비율. 공격적인 결정력을 보여주는 지표입니다.";
    } else if (type === 'score') {
        title = "평균득점(🔴) 산출 기준";
        desc = "<b>(해당 월 평균 승점 / 5.0) × 100</b><br><br>5인 게임 1위 만점(5.0)을 기준으로 본인의 평균 승점 수준을 나타냅니다. 꾸준한 기량을 보여주는 지표입니다.";
    } else if (type === 'safety') {
        title = "생존율(🟢) 산출 기준";
        desc = "<b>((경기수 - 꼴찌수) / 경기수) × 100</b><br><br>참여 경기 중 꼴찌를 하지 않고 살아남은 비율. 무너지지 않는 수비적 안정감을 보여주는 지표입니다.";
    }
