// main.js - v5.61 (아이폰 링 그래프 및 웰컴 화면 멈춤 결함 철저 픽스)
...
// 링 그래프 산정 기준 팝업 (문장 단위 줄바꿈 적용)
function showRingCriteria(type) {
let title = "", desc = "";
if (type === 'win') {
title = "승률 산출 기준";
desc = "<b>(1위 횟수 / 참여 경기수) × 100</b><br><br>해당 선수가 참여한 전체 경기 중 1위를 차지한 비율입니다.<br>공격적인 결정력을 보여주는 지표입니다.";
} else if (type === 'score') {
title = "평균득점 산출 기준";
desc = "<b>(총 획득 승점 / 참여 경기수)</b><br><br>해당 선수의 게임당 평균 획득 승점입니다.<br>꾸준한 상위권 진입 능력을 나타냅니다.";
} else if (type === 'safety') {
title = "생존율 산출 기준";
desc = "<b>((경기수 - 꼴찌수) / 경기수) × 100</b><br><br>참여 경기 중 꼴찌를 하지 않고 살아남은 비율입니다.<br>무너지지 않는 수비적 안정감을 보여주는 지표입니다.";
}
...
if (gamesOnLatestDate.length === 0) return;
...
if (dateTitle) dateTitle.innerText = 📅 ${selectedDateStr}`;
}
}
...
if(!pCountEl) return;
if(preFill) pCountEl.value = preFill.length;
const count = parseInt(pCountEl.value);
...
const stratEl = document.getElementById('strategyPlayer');
if(!stratEl) return;
const me = stratEl.value;
...
totalScore += getEarnedScore(pIdx, actual.length);
if(pIdx === 0) totalWins++;
else if(pIdx === actual.length - 1 && actual.length > 1) totalLasts++;
});

const avg = allPersonal.length > 0 ? (totalScore / allPersonal.length).toFixed(2) : "0.00";
const overallWinRate = allPersonal.length > 0 ? ((totalWins / allPersonal.length) * 100).toFixed(0) : 0;
const overallSurvival = allPersonal.length > 0 ? (((allPersonal.length - totalLasts) / allPersonal.length) * 100).toFixed(0) : 0;

const circ = 226;
const winOffset = circ - (circ * overallWinRate / 100);
const scoreOffset = circ - (circ * Math.min((parseFloat(avg) / 3) * 100, 100) / 100);
const safetyOffset = circ - (circ * overallSurvival / 100);
...
html += &lt;div class="summary-stats-wrap"&gt; &lt;div class="ring-item" onclick="showRingCriteria('win')"&gt; &lt;svg&gt; &lt;circle cx="40" cy="40" r="36" class="ring-bg"&gt;&lt;/circle&gt; &lt;circle cx="40" cy="40" r="36" class="ring-fill" stroke="var(--ring-blue)" stroke-dasharray="${circ}" stroke-dashoffset="{winOffset}"></circle>
</svg>
<div class="ring-text">
<span class="ring-val">{circ}" stroke-dashoffset="{scoreOffset}"></circle>
</svg>
<div class="ring-text">
<span class="ring-val">{circ}" stroke-dashoffset="{safetyOffset}"></circle>
</svg>
<div class="ring-text">
<span class="ring-val">${overallSurvival}%&lt;/span&gt; &lt;span class="ring-lbl"&gt;생존율&lt;/span&gt; &lt;/div&gt; &lt;/div&gt; &lt;/div>;
...
const rings = area.querySelectorAll('.ring-fill');
rings.forEach(r => {
...
if(!searchDateEl || !searchPlayerEl) return;

const mon = searchDateEl.value;
const player = searchPlayerEl.value;
...
<div><div style="font-size: 11px; font-weight: 800; color: var(--sub-text); margin-bottom: 4px;">월간순위</div><div style="font-size: 14px; font-weight: 900; color: var(--rankL);">{monthlyTotalScore}점 <span style="font-size:11px; color:var(--sub-text);">({tier.color};">{tier.name}</div></div>
...
// -------------------------------------------------------------
// [핵심 결함 완전 수정] 브라우저 환경 및 PWA 캐시에 상관없이
// 웰컴 화면은 최상단 우선순위로 즉시 강제 종료됩니다.
// -------------------------------------------------------------

// 1. 이벤트나 다른 리소스가 멈추더라도 스크립트 실행 즉시 3초 타이머가 무조건 돌아갑니다.
setTimeout(() => {
...
// 2. 앱 주요 데이터 및 UI 초기화 로직
function initBilliardApp() {
try {
...
// 3. 브라우저의 DOMContentLoaded 이벤트를 놓치는 고질적 버그를 원천 차단하기 위해
// 브라우저 로딩 상태를 직접 검사하여 안전하게 실행합니다.
if (document.readyState === 'loading') {
document.addEventListener('DOMContentLoaded', initBilliardApp);
} else {
initBilliardApp();
}
