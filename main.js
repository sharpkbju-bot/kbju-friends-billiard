...
/* 수정: V5.25 - 서버가 지원하는 기존 API(RESET, SAVE)를 활용하여 완벽한 데이터 순차 복구를 수행하도록 로직 전면 개편 */
function importData(event) {
    ...
    reader.onload = async function(e) {
        try {
            ...
            // 1. 기존 서버 데이터 초기화 (확실하게 지원하는 RESET 명령 사용)
            await fetch(GAS_URL, { method: 'POST', body: JSON.stringify({ action: "RESET" }) });

            // 2. 백업 파일의 데이터를 1번부터 끝까지 순차적으로 하나씩 개별 저장 (확실하게 지원하는 SAVE 명령 사용)
