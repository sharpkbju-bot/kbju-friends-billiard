...
function searchRecords() {
    ...
    let avgScoreRate = Math.min(100, Math.round((parseFloat(monthlyAvgScore) / 3.0) * 100));
    ...
    sArea.innerHTML = `<div class="summary-box" style="margin: 0 -5px; box-sizing: border-box; background:var(--record-bg); border:2px solid var(--record-border); border-radius:15px; padding:15px 10px;">
                           ...
                           <div class="activity-rings-row">
                               ${createActivityRing(winRate, 'var(--rank1)', 80, '승률', 'win')}
                               ${createActivityRing(avgScoreRate, 'var(--accent)', 80, '평균득점', 'score')}
                               ${createActivityRing(safetyRate, 'var(--success)', 80, '생존율', 'safety')}
                           </div>
